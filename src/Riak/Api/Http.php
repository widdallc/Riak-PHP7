...
namespace Widda\Riak\Api;

use Widda\Riak\Api;
use Widda\Riak\ApiInterface;
use Widda\Riak\Bucket;
use Widda\Riak\Command;
use Widda\Riak\Exception;
use Widda\Riak\Location;
use Widda\Riak\Node;
...
class Http extends Api implements ApiInterface
{
    ...
    protected function buildPath()
    {
        ...
        switch (get_class($this->command)) {
            case 'Widda\Riak\Command\Bucket\List':
                $this->path = sprintf('/types/%s/buckets/%s', $bucket->getType(), $bucket->getName());
                break;
            case 'Widda\Riak\Command\Bucket\Fetch':
            case 'Widda\Riak\Command\Bucket\Store':
            case 'Widda\Riak\Command\Bucket\Delete':
                $this->path = sprintf('/types/%s/buckets/%s/props', $bucket->getType(), $bucket->getName());
                break;
            case 'Widda\Riak\Command\Bucket\Keys':
                $this->path = sprintf('/types/%s/buckets/%s/keys', $bucket->getType(), $bucket->getName());
                break;
            case 'Widda\Riak\Command\Object\Fetch':
            case 'Widda\Riak\Command\Object\Store':
            case 'Widda\Riak\Command\Object\Delete':
                $this->path = sprintf('/types/%s/buckets/%s/keys/%s', $bucket->getType(), $bucket->getName(), $key);
                break;
            case 'Widda\Riak\Command\DataType\Counter\Fetch':
            case 'Widda\Riak\Command\DataType\Counter\Store':
            case 'Widda\Riak\Command\DataType\Set\Fetch':
            case 'Widda\Riak\Command\DataType\Set\Store':
            case 'Widda\Riak\Command\DataType\Map\Fetch':
            case 'Widda\Riak\Command\DataType\Map\Store':
            $this->path = sprintf('/types/%s/buckets/%s/datatypes/%s', $bucket->getType(), $bucket->getName(),
                $key);
                break;
            case 'Widda\Riak\Command\Search\Index\Fetch':
            case 'Widda\Riak\Command\Search\Index\Store':
            case 'Widda\Riak\Command\Search\Index\Delete':
                $this->path = sprintf('/search/index/%s', $this->command);
                break;
            case 'Widda\Riak\Command\Search\Schema\Fetch':
            case 'Widda\Riak\Command\Search\Schema\Store':
                $this->path = sprintf('/search/schema/%s', $this->command);
                break;
            case 'Widda\Riak\Command\Search\Fetch':
                $this->path = sprintf('/search/query/%s', $this->command);
                break;
            case 'Widda\Riak\Command\MapReduce\Fetch':
                $this->path = sprintf('/%s', $this->config['mapred_prefix']);
                break;
            case 'Widda\Riak\Command\Indexes\Query':
